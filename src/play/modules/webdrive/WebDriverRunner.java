...
public class WebDriverRunner {
	...
	this.appUrlBase = System
		.getProperty("application.url", DEFAULT_APP_URL);
	String timeoutStr = System.getProperty("webdrive.timeout",
	...
	try {
	    ...
	    System.out
		    .println("~ The application does not start. There are errors: "
			    + e);
	    System.exit(-1);
	}
    }
	...
	for (String test : tests) {
		...
		url = appUrlBase + seleniumUrlPart + "?baseUrl=" + appUrlBase
			+ "&test=/@tests/" + test
	...
	webDriver.get(appUrlBase + "/@tests/end?result="
		+ (ok ? "passed" : "failed"));
	webDriver.quit();
